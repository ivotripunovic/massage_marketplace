...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if "insecure" in settings.SECRET_KEY:
            errors.append(
                'SECRET_KEY contains "insecure". Generate a new SECRET_KEY for production.'
            )
        else:
        ...
        if not settings.ALLOWED_HOSTS or settings.ALLOWED_HOSTS == [
            ...
            warnings.append(
                "ALLOWED_HOSTS only includes localhost. Add your production domain."
            )
        else:
        ...
        if "sqlite" in db_engine:
            warnings.append(
                f"Database is SQLite ({db_engine}). Use PostgreSQL for production."
            )
        else:
        ...
        if "console" in settings.EMAIL_BACKEND:
            warnings.append(
                "EMAIL_BACKEND is console. Configure SMTP for production email delivery."
            )
        else:
        ...
        if not settings.STATIC_ROOT:
            errors.append(
                "STATIC_ROOT is not configured. Run collectstatic before deployment."
            )
        else:
        ...
        if btc == "1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa":
            warnings.append(
                "CRYPTO_BITCOIN_ADDRESS is the Satoshi genesis address (placeholder). Set your real address."
            )
        ...
        if not getattr(settings, "RATE_LIMIT_RULES", {}):
            ...
            passed.append(
                f"Rate limiting configured: {len(settings.RATE_LIMIT_RULES)} rules"
            )
